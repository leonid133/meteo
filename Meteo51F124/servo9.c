...
*/			
	...
	u = u+((V-V_pr)*FREQT0-u)*.5;//u = u+((V-V_pr)*FREQT0-u)/FREQT0*K;
	...
	{
		V_zad = V_zad + (tmp - V_zad)/FREQT0*200.;
	}
	else 
	{
		V_zad = V_zad + ((tmp*0.01) - V_zad);
		Xtmp = 0.5;
	}	
	...
	if(fabs(V)>2.) // ((A_zad-A)/fabs(A_zad-A)+(V_zad-V)/fabs(V_zad-V)==0))
	{
		Xtmp = Xtmp + (Xtmp*0.9 - Xtmp);
	}

	if(fabs(V)<2.)
	{
		Xtmp = Xtmp + (Xtmp*1.1 - Xtmp);
	}
