...
#include <math.h> 
//system
#define SYSCLK      24500000  // (24856840L * 1 / 1) 
#define FREQT0 1166 //Hz
#define PCACLK	1166
...
xdata unsigned int LirData, LirTarget;
xdata unsigned char shim_pr=0;
xdata float A=0.0, A_zad=0.0, V=0.0, V_zad=0.0, V_pr=0.0, Accel=0, A_pr=0.0, tau=0.0;
...
//ADC
		if(DavlFlag==1)
		{
			...
			EA = 1; 
			DavlFlag = 0;
			PressureFl = Pressure + (PressureFl - Pressure)*0.15;
		}
		EIP2      |= 0x02;
//Veter
		if(VeterFlag==1)
		...
		if(u0timer>0xf)
		{
			...
			if(wBFM > 3)
			{
			   unsigned char chtmp = 0;
				...
				if(chtmp == BuferFromModem[3] && BuferFromModem[0] == 0x6e)
				{
					int tmp=0;
			...
			BufferInModem[0] |= 0x40; //кодовый байт
			OutModem4(LatGnd, 1);
			OutModem4(LonGnd, 5);
   		...
   		BufferInModem[15] = 0;
			...
			BufferInModem[3] = 0;
   		for (i = 0; i < 3; i++ )
		     	BufferInModem[3] = BufferInModem[3] ^ BufferInModem[i];
...
#define K 50.8
	...
	if ((A > 270) && (tmp < 90))
	{
		A = A+(tmp+360-A)/PCACLK*K;
		if (A > 360)
			A = A-360;
	}
	else if ((tmp > 270) && (A < 90))
	{
		A = A+(tmp-A-360)/PCACLK*K;
		if (A < 0)
			A = A+360;
	}
	else
	...
	if ((A_pr > 270) && (A < 90))
		V = V+((A+360-A_pr)*PCACLK-V)/PCACLK*K;
	else if ((A > 270) && (A_pr < 90))
		V = V+((A-A_pr-360)*PCACLK-V)/PCACLK*K;
	else
		V = V+((A-A_pr)*PCACLK-V)/PCACLK*K;
	if (V > 200) 
		V = 200;
	else if (V < -200) 
		V = -200;
	...
	tau = (V_zad-V)/200*0.000025;
	if (tau > 0.000025)
		tau = 0.000025;
	else if (tau < -0.000025)
		tau = -0.000025;

	shim = 0xff-0x3f*fabs(tau)/0.000025;	
	if (tau > 0)
	{
		PCA0CN    = 0x40;
	   PCA0MD    = 0x01;
