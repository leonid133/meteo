...
sbit LIR_SCK = P2^1;
xdata unsigned int LData;
xdata float A = 0, A_pr = 0, A_zad = 0, A_zad_new = 0, A_zad_pr = 0, V = 0, V_pr = 0, V_zad = 0, V_zad_pr = 0, u = 0, u_zad = 0/*, Accel=0*/, tau=0.0, K = 200.;
...
//-----------
		...
		//---------------------------------------------------------------------
//		if(u0timer > (3*FREQT0))
		if((flTransmiter == 0) && (u0timer > FREQT0))
		{
			u0timer = 0;
			if(wBFM > 4)
			{
...
{
	xdata float tmp;
	xdata unsigned char shim;
	xdata unsigned int i_t0;
...
*/			
	...
	if ((A > 270) && (tmp < 90))
	{
		A = tmp+360;//A = A+(tmp+360-A)/FREQT0*K;
		if (A > 360)
			A = A-360;
	}
	else if ((tmp > 270) && (A < 90))
	{
		A = tmp-360;//A = A+(tmp-A-360)/FREQT0*K;
		if (A < 0)
			A = A+360;
	}
	else
		A = tmp;//A = A+(tmp-A)/FREQT0*K;

	if ((A_pr > 270) && (A < 90))
		V = (A+360-A_pr)*FREQT0;//V = V+((A+360-A_pr)*FREQT0-V)/FREQT0*K;
	else if ((A > 270) && (A_pr < 90))
		V = (A-A_pr-360)*FREQT0;//V = V+((A-A_pr-360)*FREQT0-V)/FREQT0*K;
	else
		V = (A-A_pr)*FREQT0;//V = V+((A-A_pr)*FREQT0-V)/FREQT0*K;
	A_pr = A;
	...
	if (V > Vmax) 
		...
		V = -Vmax;
/*
	if ((A_zad > 270) && (A_zad_new < 90))
		A_zad = A_zad + (A_zad_new + 360 - A_zad)/FREQT0*K;
	else if ((A_zad_new > 270) && (A_pr < 90))
		A_zad = A_zad + (A_zad_new - 360 - A_zad)/FREQT0*K;
	else
		A_zad = A_zad + (A_zad_new - A_zad)/FREQT0*K;
*/
	A_zad = A_zad_new;
	...
	while (tmp < -180)
     	tmp += 360;
	tmp = tmp/UgolMaxV*Vmax;	
	if(tmp > Vmax)	
		tmp = Vmax;
	else if(V_zad < -Vmax)	
		tmp = -Vmax;
	
//	V_zad = V_zad + (tmp - V_zad)/FREQT0*K;
	
	if(fabs(tmp) > fabs(V_zad))
	{
		V_zad = V_zad + (tmp - V_zad)/FREQT0*K;
	}
	else if((fabs(V_zad)+0.1)<fabs(tmp))
	{
		V_zad = V_zad + ((tmp*0.01) - V_zad)/FREQT0*1100.;
	}
	else
	{
		V_zad = V_zad + (tmp - V_zad)/FREQT0*1100.;
	}
	
	
/*
	if(fabs(A_zad - A)<UgolMaxV)
	{
		V_zad = V_zad*0.5;
	}*/


	tau = (V_zad-V)/Vmax*taumax;
	if (tau > taumax)
		tau = taumax;
	else if (tau < -taumax)
		tau = -taumax;
/*
	LirData = LData;
		...
		if(LirData==0x0FFF)//при отключенном лир, и в точке 0 град, воздействие отсутствует
		{
			F=0x03;
		}
//ШИМы на машинку--------------------------
		if(F==0xA1)
		{
			Fdelta =  delta - Fdelta;
			Fdelta = delta;
			shim = 0xff-0x3f*fabs(tau)/taumax;
			PCA0CPH1  = 0xFF;
			PCA0CPH0  = shim;
		}
		else if(F==0xA2)
		...
		else if(F==0xC1)
		{
			...
			if(fabs(V)>(Vmax/2.))
			{
				shim = 0xff-0x3f*fabs(taumax/2.)/taumax;
				PCA0CPH0  = 0xff;
				//PCA0CPH1  = shim;
				PCA0CPH1  = 0xff;
		...
		else if(F==0xC2)
		{
			...
			if(fabs(V)>(Vmax/2.))
			{
				shim = 0xff-0x3f*(taumax/2.)/taumax;
				PCA0CPH1  = 0xff;
				//PCA0CPH0  = shim;
				PCA0CPH0  = 0xff;
			}
...
/*
	...
	//---------------------------------------------
	tmp = (A_zad-A)*10;///UgolMaxV*Vmax;
	V_zad = V_zad+(tmp-V_zad)/FREQT0*K; //-360 ... +360гр
...
/*
	...
	if ((A > 270) && (tmp < 90))
	{
		A = A+(tmp+360-A)/FREQT0*(K*0.1);
		if (A > 360)
			A = A-360;
	}
	else if ((tmp > 270) && (A < 90))
	{
		A = A+(tmp-A-360)/FREQT0*(K*0.1);
		if (A < 0)
			A = A+360;
	}
	else
		A = A+(tmp-A)/FREQT0*(K*0.1);

	V = V+((A-A_pr)*FREQT0-V)/FREQT0*(K*0.1);
	A_pr = A;
	u = u+((V-V_pr)*FREQT0-u)/FREQT0*(K*0.1);
	V_pr = V;

	//---------------------------------------------
	tmp = (A_zad-A)/UgolMaxV*Vmax;
	if(tmp > Vmax)	
		tmp = Vmax;
	else if(tmp < -Vmax)	
		tmp = -Vmax;
	V_zad = V_zad+(tmp-V_zad)/FREQT0*(K*0.1); //-360 ... +360гр
	u_zad = u_zad+((V_zad-V_zad_pr)*FREQT0-u_zad)/FREQT0*(K*0.1);
	if(u_zad> Umax)	
		u_zad = Umax;
	else if(u_zad < -Umax)	
		u_zad = -Umax;
	...
	tau = (u_zad-u)/Umax*taumax;
	tmp = (V_zad-V)/Vmax*taumax;
	if(tau>tmp)
		tau = tmp;
