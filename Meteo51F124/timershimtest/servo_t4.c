...
float A = 0, A_pr = 0, A_zad = 0, A_zad_pr = 0, V = 0, V_pr = 0, V_zad = 0, V_zad_pr = 0, u = 0, u_zad = 0, Accel=0, tau=0.0, K = 1.5;
...
if(flShim == 0)
{
		...
		if ((A_pr > 270) && (A < 90))
			V = V+((A+360-A_pr)*FREQT0-V)/KK; 
		else if ((A > 270) && (A_pr < 90))
			V = V+((A-A_pr-360)*FREQT0-V)/KK; 
		else
			V = V+((A-A_pr)*FREQT0-V)/KK; 
		A_pr = A;
