...
#include <math.h> 
//system
#define SYSCLK      24685000  // (24856840L * 1 / 1) 
#define FREQT0 	  947 //Hz
...
float A = 0, A_pr = 0, A_zad = 0, A_zad_pr = 0, V = 0, V_pr = 0, V_zad = 0, V_zad_pr = 0, u = 0, u_zad = 0, Accel=0, tau=0.0, K = 1.0;
/*
typedef union INT {                   
   unsigned int i;
   unsigned char b[2];
...
*/
int  UgolMaxV = 4, Vmax = 200, Umax = 2;
float taumax = 0.000025;
...
#define cT 		0xD025 //0xA04B //0x3f25 
#define ctau   0X17ED //0x3036 
...
{
	float tmp;
	static unsigned int shim, KK;
	char SFRPAGE_SAVE = SFRPAGE;
	static char flShim = 0;
...
if(flShim == 0)
{
		...
		LIR_SCK = 0;	shim++;shim++;shim++;shim++;shim++;
...
*/
		...
		if ((A_pr > 270) && (A < 90))
			V = V+((A+360-A_pr)*FREQT0-V)*K; 
		else if ((A > 270) && (A_pr < 90))
			V = V+((A-A_pr-360)*FREQT0-V)*K; 
		else
			V = V+((A-A_pr)*FREQT0-V)*K; 

		A_pr = A;

		u = u+((V-V_pr)*FREQT0-u)/KK;
		V_pr = V;
		...
		u_zad = u_zad+((V_zad-V_zad_pr)*FREQT0-u_zad)/KK;
		if(u_zad>Vmax)u_zad=Vmax;
		else if(u_zad<-Vmax)u_zad=-Vmax; 
/*
		if ((V_zad_pr > 90) && (V_zad < -90))
			u_zad = u_zad+((V_zad+360-V_zad_pr)*FREQT0-u_zad)/KK; 
		else if ((V_zad_pr > 90) && (V_zad < -90))
			u_zad = u_zad+((V_zad-V_zad_pr-360)*FREQT0-u_zad)/KK; 
		else
			u_zad = u_zad+((V_zad-V_zad_pr)*FREQT0-u_zad)/KK;
		...
		*/
		...
		tmp = (u_zad-u)/Vmax*ctau + (7.0*V_zad-V)/Vmax*ctau;	//???
